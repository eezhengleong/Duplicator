...
def checkTime(Secs, Mins, Hours): #return the time that was incremented by 1 seconds
    if Secs == 60:
        Secs = 00
        Mins += 1
...
def editFile(dest, stringList): #method to rewrite txt file
    txt_file = open(dest, 'w')
    new_file_contents = "".join(stringList)
    txt_file.write(new_file_contents)
    txt_file.close

def editLine(TXTYear, TXTMonth, TXTDay, TXTHour, TXTMins, TXTSecs, dest, line_num, x): #changes made before rewriting txt file
...
def DateAndTime(src): #getting date, time and panel name 
...
def duplicateFolder(YYYY, MM, DD, HH, MIN, SS, frontName, src, opath): #duplication mechanism
...
def find_param(src): #method to call 2 methods to find line number to chg in txt and date, time, panel of source folder
...
def main(src, opath, lineList, formating): #main method to call for duplication
