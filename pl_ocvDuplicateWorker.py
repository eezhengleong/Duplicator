...
import pl_ocvDuplicate, psycopg2, random, string, configparser
...
class DuplicateObject(QObject):
    ...
    def setParam(self, pathname, copies, outname, params, file_list, queries, ocv, ocv_path, defaultpath): #set parameter before run
        self.pathname, self.copies, self.outname, self.params, self.file_list, self.queries, self.OCV, self.OCVpath, self.defaultpath = pathname, copies, outname, params, file_list, queries, ocv, ocv_path, defaultpath
        self.copied = 0
    ...
    def run(self): #method that will be executed when thread.start() is called
        ...
        dbname = pl_ocvDuplicate.extractName(self.pathname)
        for i in range(self.copies):
            if self.threadactive:
                name = pl_ocvDuplicate.main(self.pathname, self.outname, dbname, self.file_list, self.conn, self.cur, self.queries["q4"],self.OCV, self.OCVpath, self.defaultpath)
